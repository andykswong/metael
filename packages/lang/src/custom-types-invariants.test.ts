...
const run = (src: string, opts?: object) => evaluateProgram(src, { host: new PlainStorageHost(), env: new RecordingHostEnv(), ...opts });
...
describe('the protocol preserves the load-bearing invariants', () => {
  ...
  it('DETERMINISM: same source + seed → identical result for a buffer-generator mix', () => {
    const src = 'const a = f32(3, (i) => rand() + i); a[0] + a[1] + a[2]';
  ...
  it('NEVER-THROWS: a generator that recurses infinitely fails closed (ML-LANG-BUDGET)', () => {
    ...
    const r = evaluateProgram(src, { host: new PlainStorageHost(), env: new RecordingHostEnv(), maxDepth: 20 });
