...
import type { BindableHostEnv, Arg, HostValue, SourceSpan, ReactiveHost } from '@metael/lang';
...
/** The `HostEnvironment` that supplies the compute vocabulary: it resolves the `gpu` / `gpuReduce` /
 *  `gpuHistogram` heads to reactive compute resources and declines every other call. Construct it with the
 *  device deps, then `bindHost` it to the reactive host before evaluating a program. */
export class GpuHostEnv implements BindableHostEnv, Disposable {
  ...
  /** Construct the host env with the device dependencies the engine acquires backends through. The engine
   *  itself is created lazily on {@link GpuHostEnv.bindHost} (it needs the reactive host). */
  constructor(deps: GpuEngineDeps) { this.deps = deps; }
  /** Bind the reactive host and create the underlying {@link GpuEngine}. Called once by the runtime before
   *  a program is evaluated, so `resolveCall` has a host-backed engine to dispatch on. */
  ...
  /** Dispose the underlying engine (freeing its devices + resident buffers). Safe to call when no host was
   *  bound (the engine is then `null`). */
