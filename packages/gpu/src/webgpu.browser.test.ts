...
describe('@metael/gpu — real WebGPU dispatch (Chromium)', () => {
  ...
  it('the emitted WGSL actually COMPILES on a real device (catches i32/f32 type errors a substring test misses)', async () => {
    const gpu = (navigator as unknown as { gpu?: GPU }).gpu;
    if (!gpu) { expect(true).toBe(true); return; }
    const adapter = await gpu.requestAdapter();
    if (!adapter) { expect(true).toBe(true); return; }
  ...
  it('the f16 WGSL compiles on a real device that supports shader-f16 (else skips)', async () => {
    const gpu = (navigator as unknown as { gpu?: GPU }).gpu;
    if (!gpu) { expect(true).toBe(true); return; }
    const adapter = await gpu.requestAdapter();
    if (!adapter || !adapter.features.has('shader-f16')) { expect(true).toBe(true); return; }   // no f16 → skip
  ...
  it('the reduce WGSL (workgroup-shared tree reduction) compiles on a real WebGPU device (else skips)', async () => {
    const gpu = (navigator as unknown as { gpu?: GPU }).gpu;
    if (!gpu) { expect(true).toBe(true); return; }
    const adapter = await gpu.requestAdapter();
    if (!adapter) { expect(true).toBe(true); return; }
  ...
  it('the histogram WGSL (atomic scatter) compiles on a real WebGPU device (else skips)', async () => {
    const gpu = (navigator as unknown as { gpu?: GPU }).gpu;
    if (!gpu) { expect(true).toBe(true); return; }
    const adapter = await gpu.requestAdapter();
    if (!adapter) { expect(true).toBe(true); return; }
